...
st.title(APP)
st.caption('Build V6 • Rule-based SMC engine • Upstox V3');st.caption('Pre-breakout intelligence • SMC • market structure • risk-aware setups')
...
def smc_analysis(d, pivot=3):
    """Rule-based SMC-style annotations from OHLC only.
    These are transparent heuristics, not broker/order-flow data.
    """
    x=d.copy().reset_index(drop=True)
    n=len(x)
    x['swing_high']=False; x['swing_low']=False
    if n >= 2*pivot+1:
        for i in range(pivot,n-pivot):
            x.loc[i,'swing_high']=x.high.iloc[i] >= x.high.iloc[i-pivot:i+pivot+1].max()
            x.loc[i,'swing_low']=x.low.iloc[i] <= x.low.iloc[i-pivot:i+pivot+1].min()

    swing_highs=x.index[x.swing_high].tolist()
    swing_lows=x.index[x.swing_low].tolist()
    events=[]
    last_high=None; last_low=None; trend=None
    for i in range(n):
        c=float(x.close.iloc[i])
        if x.swing_high.iloc[i]:
            last_high=float(x.high.iloc[i])
        if x.swing_low.iloc[i]:
            last_low=float(x.low.iloc[i])
        if last_high is not None and c > last_high and (not events or events[-1]['level'] != last_high or events[-1]['index'] != i):
            newtrend='bullish'
            events.append({'index':i,'type':'BOS' if trend in (None,'bullish') else 'CHOCH','direction':'bullish','level':last_high})
            trend=newtrend; last_high=None
        elif last_low is not None and c < last_low and (not events or events[-1]['level'] != last_low or events[-1]['index'] != i):
            newtrend='bearish'
            events.append({'index':i,'type':'BOS' if trend in (None,'bearish') else 'CHOCH','direction':'bearish','level':last_low})
            trend=newtrend; last_low=None

    # Fair Value Gaps: bullish when current low > high two candles back;
    # bearish when current high < low two candles back.
    gaps=[]
    for i in range(2,n):
        if float(x.low.iloc[i]) > float(x.high.iloc[i-2]):
            gaps.append({'index':i,'type':'Bullish FVG','low':float(x.high.iloc[i-2]),'high':float(x.low.iloc[i])})
        if float(x.high.iloc[i]) < float(x.low.iloc[i-2]):
            gaps.append({'index':i,'type':'Bearish FVG','low':float(x.high.iloc[i]),'high':float(x.low.iloc[i-2])})

    # Order block heuristic: last opposite candle before a displacement candle
    # that breaks a recent swing level. The zone is the opposite candle body/wick.
    obs=[]
    for e in events:
        i=e['index']
        if i < 1: continue
        j=i-1
        if e['direction']=='bullish' and x.close.iloc[j] < x.open.iloc[j]:
            obs.append({'index':j,'type':'Bullish Order Block','low':float(x.low.iloc[j]),'high':float(x.open.iloc[j])})
        elif e['direction']=='bearish' and x.close.iloc[j] > x.open.iloc[j]:
            obs.append({'index':j,'type':'Bearish Order Block','low':float(x.open.iloc[j]),'high':float(x.high.iloc[j])})

    # Liquidity sweep: wick takes a recent swing then closes back through it.
    sweeps=[]
    for i in range(1,n):
        recent=x.iloc[max(0,i-20):i]
        rh=float(recent.high.max()); rl=float(recent.low.min())
        if float(x.high.iloc[i]) > rh and float(x.close.iloc[i]) < rh:
            sweeps.append({'index':i,'type':'Buy-side liquidity sweep','level':rh})
        elif float(x.low.iloc[i]) < rl and float(x.close.iloc[i]) > rl:
            sweeps.append({'index':i,'type':'Sell-side liquidity sweep','level':rl})

    latest=[]
    for arr in (events,gaps,obs,sweeps):
        latest.extend(arr[-8:])
    latest=sorted(latest,key=lambda z:z['index'])[-20:]
    return x,events,gaps,obs,sweeps,latest
...
if page=='Dashboard':
    ...
    if df is None or df.empty:st.info('Run the Live Scanner first.')
        ...
        x,fig=setup_chart(TOKEN,str(r.instrument_key),s,240)
        sx,events,gaps,obs,sweeps,latest=smc_analysis(x,3)
        ...
        sh=sx[sx.swing_high].tail(12); sl=sx[sx.swing_low].tail(12)
        if not sh.empty:
            fig.add_trace(go.Scatter(x=sh.timestamp,y=sh.high,mode='markers',name='Swing High',marker={'size':7,'symbol':'triangle-up'}))
        if not sl.empty:
            fig.add_trace(go.Scatter(x=sl.timestamp,y=sl.low,mode='markers',name='Swing Low',marker={'size':7,'symbol':'triangle-down'}))
        for g in gaps[-6:]:
            i=g['index']; x0=sx.timestamp.iloc[i-2]; x1=sx.timestamp.iloc[-1]
            fig.add_shape(type='rect',x0=x0,x1=x1,y0=g['low'],y1=g['high'],line={'width':0},opacity=.10)
        for o in obs[-5:]:
            i=o['index']; x0=sx.timestamp.iloc[i]; x1=sx.timestamp.iloc[-1]
            fig.add_shape(type='rect',x0=x0,x1=x1,y0=o['low'],y1=o['high'],line={'width':1},opacity=.12)
        for e in events[-8:]:
            i=e['index'];
            fig.add_annotation(x=sx.timestamp.iloc[i],y=e['level'],text=e['type'],showarrow=True,arrowhead=2)
        for sw in sweeps[-5:]:
            i=sw['index']; fig.add_annotation(x=sx.timestamp.iloc[i],y=sw['level'],text='Liquidity sweep',showarrow=True,arrowhead=1)
        fig.update_layout(template='plotly_dark',height=620,margin={'l':10,'r':10,'t':45,'b':10},title=f'{s} • SMC / Market Structure V6')
        st.plotly_chart(fig,use_container_width=True)

        st.markdown('#### SMC signal summary')
        a,b,c,d=st.columns(4)
        a.metric('BOS',len(events))
        b.metric('FVG',len(gaps))
        c.metric('Order Blocks',len(obs))
        d.metric('Liquidity Sweeps',len(sweeps))
        if latest:
            sm=pd.DataFrame(latest)
            sm['Date']=sm['index'].map(lambda i: sx.timestamp.iloc[i].strftime('%Y-%m-%d'))
            sm=sm[['Date','type']].rename(columns={'type':'Signal'})
            st.dataframe(sm.tail(15).iloc[::-1],use_container_width=True,hide_index=True)
        else:
            st.info('No SMC event was detected in the selected history. The app does not invent a signal.')
        ...
        st.caption('SMC uses transparent OHLC-based heuristics for FVG, order blocks, BOS, CHOCH and liquidity sweeps. It is not true Level-2/order-flow data, and no signal is fabricated.')
elif page=='Settings':
