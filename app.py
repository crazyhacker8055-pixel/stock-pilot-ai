...
def evaluate(d,price=None,volume=None):
    ...
    prior=x.iloc[:-1];breakout=float(prior.tail(252).high.max());last_close=float(prior.iloc[-1].close);confirmed=last_close>breakout;above=p>breakout;dist=(p/breakout-1)*100 if breakout else np.nan;near=not confirmed and .97*breakout<=p<=1.03*breakout
...
def fields(item):
    ...
    lo=item.get('live_ohlc') or {};p=item.get('last_price') or lo.get('close');v=lo.get('volume') or item.get('volume');pr=(item.get('prev_ohlc') or {}).get('close') or item.get('prev_close_price')
...
def all_quotes(_token,keys):
    u=Upstox(_token);out={};ks=list(keys)
    for i in range(0,len(ks),500):out.update(u.quotes(ks[i:i+500]))
    return out
@st.cache_data(ttl=45,show_spinner=False)
def index_quotes(_token,keys):return Upstox(_token).ohlc(list(keys))
...
def candidates(uni,quotes,limit):
    ...
    for r in uni.itertuples(index=False):
        q=quotes.get(str(r.instrument_key));p,v,_=fields(q)
...
with st.sidebar:
    ...
    st.divider();st.success('🟢 UPSTOX CONNECTED') if TOKEN else st.error('🔴 UPSTOX TOKEN REQUIRED')
...
for c,l in zip(cs,labels):
    p,_,pr=fields(iq.get(iks[l]));d=(p-pr)/pr*100 if p is not None and pr else None
...
if page=='Dashboard':
    ...
    if st.button('🚀 RUN FULL NSE LIVE SCAN',type='primary',use_container_width=True):
        ...
        if df.empty:st.warning('No candidates returned. No fake data was inserted.')
